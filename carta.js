function generarCodigoInterno() {
  ...
  let ultimo =
    parseInt(
      localStorage.getItem("ultimoCodigoInterno")
    ) || 0;
...
function cargarXML() {
  ...
  reader.onload = function (e) {
    ...
    const fechaFactura = fechaHora
  ? fechaHora.split("T")[0]
  : "—";

window.fechaFactura = fechaFactura;
    ...
    const direccionArray = [direccion, municipio, departamento]
      .filter(v => v && v.trim() !== "");
...
const numero =
  nodoAut?.getAttribute("Numero") || "";

agregarItem();
guardarEnBD(serie, numero);
...
function guardarEnBD(serie, numero) {
...
const vendedor =
  ...
  const nuevaFactura = {
    ...
    cliente:
      document.getElementById("nombreReceptor")
      ?.textContent || "—",

    nit:
      document.getElementById("nitReceptor")
      ?.textContent || "—",
  ...
  const existeLocal = facturas.some(f =>
