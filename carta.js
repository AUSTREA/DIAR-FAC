...
function cargarXML() {
  ...
  reader.onload = function (e) {
    ...
    const nodoAut =
      ...
      //nuevo codigo para la autorizacion
try {
  ...
  const existe = facturas.some(f =>
    f.serie === serie && f.numero === numero
  );

  if (!existe && serie && numero) {
...
// ==========================
// GUARDAR SOLO SERIE + NÚMERO EN BD
// ==========================
...
function guardarEnBD(serie, numero) {
  ...
  // EVITAR DUPLICADOS
  const existe = facturas.some(f =>
    f.serie === nuevaFactura.serie &&
    f.numero === nuevaFactura.numero
  );
